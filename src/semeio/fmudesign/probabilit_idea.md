...
**Probabilit** is a Python library for Monte Carlo analysis, specializing in Latin Hypercube Sampling (LHS) with Iman–Conover correlation. It provides an open-source alternative to commercial Excel add-ins like @Risk, ModelRisk, and Crystal Ball.

Why not use existing libraries like PyMC or SALib? While both are excellent tools, they serve different purposes:
...
- Efficient Latin Hypercube Sampling
- Intuitive correlation specification
- A user-friendly API suitable for both interactive notebooks and production code

The hard math was already solved in the last version of fmudesign (https://github.com/equinor/semeio/blob/main/CHANGELOG.md#1180) - now we "just" need a good API. This could be useful in:

- fmudesign
- ert
- Cost risk projects
- Anywhere else needing correlated sampling
